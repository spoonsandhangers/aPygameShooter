...
class Soldier(pygame.sprite.Sprite):
	def __init__(self, char_type, x, y, scale, speed):
		...
		animation_types = ['walk', 'idle', 'jump']
		for animation in animation_types:
			...
			# loading the image
				img = pygame.image.load(f'img/{self.char_type}/{animation}/alien{i}.png')
	...
	def move(self, moving_left, moving_right):
		...
		if self.jump == True and self.in_air == False:
			self.vel_y = -11 # moving up is in -ve direction
			self.jump = False
			self.in_air = True
		...
		if self.vel_y > 10:
			self.vel_y
		dy += self.vel_y
		# check collision with floor (line)
...
while run:
	...
	# it only runs if the player is alive.
	if player.alive:
		if player.in_air:
